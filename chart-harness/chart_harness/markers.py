...
def _glyph(mask, band, pad=6):
    ...
    if not (2 < box[2] - box[0] <= MAX_GLYPH_SIDE and 2 < box[3] - box[1] <= MAX_GLYPH_SIDE):
        return None
    return box
...
def legend_glyphs(gray, plot_bbox, count):
    ...
    mask = _ink(gray)
    candidates = []
    for band in _rows(mask, plot_bbox):
        ...
        if width < MIN_ROW_WIDTH or width < MIN_ROW_ASPECT * height:
            continue
        glyph = _glyph(mask, band)
        if glyph:
    ...
    for entry in candidates:
        ...
        if columns and centre - columns[-1][0] <= COLUMN_TOLERANCE:
...
def mine_templates(image_path, points, outdir, window=MINED_WINDOW):
    ...
    for point in points:
        ...
        if side < 3 or side > MAX_GLYPH_SIDE:
            continue
...
def crowding(gray, x, y, windows=OVERLAP_WINDOWS, glyph_ink=None):
