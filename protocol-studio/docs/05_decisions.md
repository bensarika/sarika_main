...
| ADR-017 | **Decided (Q5+Q6):** corpus is an S3 bucket/prefix (path pending from Ben; read access to be granted to `devin-deployer` if outside `protocol-studio-*`); atopic dermatitis is the only launch indication. Ingestion is still keyed by `indication` so a second one is additive | Real corpus from day one; AD-only keeps the acceptance gate small | Dropbox; attached protocols only |
