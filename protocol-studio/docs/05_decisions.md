...
| ADR-017 | **Decided (Q5+Q6):** corpus is `s3://sarika-main-fs/` (us-east-2; empty as of 2026-09-06 — Ben to upload FDA reviews / protocols / SAPs, prefix layout proposed in `docs/04_workflows.md` ingestion section). `devin-deployer` has inline policy `ProtocolStudioCorpusReadOnly` (ListBucket/GetBucketLocation on the bucket, GetObject on `*`); the app never writes to this bucket — derived JSON/MD goes to `protocol-studio-*`; atopic dermatitis is the only launch indication. Ingestion is still keyed by `indication` so a second one is additive | Real corpus from day one; AD-only keeps the acceptance gate small | Dropbox; attached protocols only |
| ADR-018 | **Decided (Q9+Q11):** OpenAI is the only provider at launch; the gateway **fails closed** — `confidential`/`restricted` data classes are never sent to any provider until an admin approves that provider for the class in Admin › Models & providers. Generic OpenAI-compatible adapter remains so xAI/Muse can be added without code | Compliance first; one adapter to harden | Grok/Muse at launch; confidential-by-default |
| ADR-019 | **Decided (Q10):** protocol authoring first; the shared trial model carries SAP fields from Pilot 1 (estimands incl. intercurrent-event strategies, analysis sets, hypotheses/multiplicity, analysis methods, missing-data handling) so the SAP editor is a thin view over the same model in a later pilot | Avoids a model migration when SAP lands; protocol §10 (statistics) and the SAP render from one source | SAP editor in v1; protocol-only model |
...
| ~~Q5~~ | *Answered → ADR-017.* **Corpus location:** do the FDA reviews / protocols / SAPs already sit in a Dropbox folder or S3 bucket? Link(s)? | Library pilot should ingest your real corpus, not just the 4 attached protocols | Start with the attached protocols + public FDA reviews for AD drugs |
| ~~Q6~~ | *Answered → ADR-017.* **Indications:** AD first (per toolkit). Any second indication to keep the model honest (e.g. psoriasis, asthma)? | Avoids AD-specific assumptions leaking into the general model | AD only in Phase 1 |
...
| ~~Q9~~ | *Answered → ADR-018.* **Model-provider data handling:** which providers may receive which data classes (public library docs / your confidential protocols, IBs, SAPs / restricted)? OpenAI API data is not used for training by default, but zero-data-retention needs an enterprise agreement. | Compliance | **Fail closed**: only `public` data goes to any provider until you approve a provider for `confidential` in Admin |
| ~~Q11~~ | *Answered → ADR-018.* **Model vendors at launch:** OpenAI is seeded. Which others should be wired in Phase 1 (Grok/xAI, Anthropic, Google Gemini, Muse — please share Muse's API docs/base URL)? Keys can be added later in Admin. | Each vendor is one adapter; Muse is unknown to me and needs its API reference | OpenAI + generic OpenAI-compatible adapter (covers many vendors incl. xAI) |
| ~~Q10~~ | *Answered → ADR-019.* **SAP scope in Phase 1:** shared trial definition with the SAP as a separately versioned artifact (recommended) — do you need SAP authoring in Phase 1 or only the protocol? | Effort | Protocol first; SAP artifact in Phase 3 |
